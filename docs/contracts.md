...
Priorities retain a nullable `actor`; unknown identity never removes a priority. `analysis.criteria` preserves exact transcript provenance for the common unweighted rubric, and new reviews require one `criterionAssessments` entry for every option/criterion pair. These fields are optional only when reading older saved records. `analysis.meetingRevision` invalidates cached extraction after corrections. Attributed stakeholder replies use `Evidence.relevance.method: DIRECT` so they remain citable independently of search ranking; a direct relationship is not a truth score.

The workflow approves only the currently recommended option, since its conditions and action sequence are the ones reviewed. After the immutable approval is stored, `indexApprovedDecision` publishes searchable choice context through `SourceStore`. Indexing is idempotent and can be retried independently without rewriting or undoing the approval.
